...
def search_product():
    ...
    with DB.engine.connect() as conn:
        products = conn.execute(sql_request).fetchall()
    return render_template("product/search_result.html", products=products)
...
def order_delivered():
    select_order_delivered = text(
        "SELECT "
        "p.product_name, "
        "SUM(co.quantity_of_product) AS total_ordered, "
        "SUM(COALESCE(d.quantity_of_product, 0)) AS total_delivered "
        "FROM "
        "product p "
        "INNER JOIN "
        "cosmetic_order co ON p.product_id = co.product_id "
        "LEFT JOIN "
        "delivery d ON co.cosmetic_order_id = d.cosmetic_order_id "
        "GROUP BY "
        "p.product_name;"
    )
    with DB.engine.connect() as conn:
        products = conn.execute(select_order_delivered).fetchall()
    return render_template("product/order-delivered.html", products=products)
...
def delete_product(id):
    ...
    try:
        ...
        flash(
            "На этот товар есть заказы. При удалении они удаляться"
